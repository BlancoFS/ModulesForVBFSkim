...
float RecoMELA_VBF_VH(float nCleanJet, float nLepton, float PuppiMet_pt, float PuppiMet_phi, float Lepton_pt0, float Lepton_pt1, float Lepton_phi0, float Lepton_phi1, float Lepton_eta0, float Lepton_eta1,float CleanJet_pt0, float CleanJet_pt1, float CleanJet_phi0, float CleanJet_phi1, float CleanJet_eta0, float CleanJet_eta1, float Lepton_pdg0, float Lepton_pdg1){
  ...
  if(nCleanJet >= 2 && nLepton > 1){
    ...
    float vh = RecoLevel_me_WH_hsm + RecoLevel_me_ZH_hsm;
    
    return (float)vbf*vbf/(vbf*vbf + 1e16 * (vh*vh));
